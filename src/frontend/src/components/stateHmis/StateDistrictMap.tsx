...
interface Props { stateName: string; counts: Record<string, number>; districts: string[]; }
...
export function StateDistrictMap({ stateName, counts, districts }: Props) {
  ...
  return <div className="space-y-3"><svg viewBox={`${x0} ${y0} ${width} ${height}`} className="h-[430px] w-full rounded-2xl bg-slate-100">
    {features.map((feature) => { const dataName = districts.find((district) => normalize(district) === normalize(feature.properties.district_name)); const count = dataName ? counts[dataName] ?? 0 : 0; return <path key={feature.id} d={feature.path} fill={dataName ? color(count, max) : "#e5e7eb"} stroke="#fff" strokeWidth="1" vectorEffect="non-scaling-stroke"><title>{feature.properties.district_name}: {dataName ? `${count} flagged month(s)` : "not in upload"}</title></path>; })}
