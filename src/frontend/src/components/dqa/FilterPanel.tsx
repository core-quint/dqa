...
export const dropdownTriggerClass =
  "group inline-flex min-h-[2.25rem] items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-[13px] font-semibold text-slate-700 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition hover:border-slate-300 hover:bg-slate-50";

const dropdownPanelClass =
  "absolute top-full z-[9999] mt-2 max-h-[70vh] min-w-[280px] max-w-[92vw] overflow-y-auto rounded-[24px] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(255,255,255,0.95),rgba(248,250,252,0.92))] p-4 shadow-[0_26px_60px_rgba(15,23,42,0.16)] backdrop-blur-xl sm:max-w-[360px]";
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {hasBadge ? (
          <span className="rounded-full bg-slate-950 px-1.5 py-0.5 text-[10px] font-bold leading-none tabular-nums text-white">
            {badge}
          </span>
        ) : null}
        <ChevronDown
    ...
    <div
      className={[
        ...
        fullWidth
          ...
          : "inline-flex min-h-[2.25rem] items-center",
      ].join(" ")}
    >
      {options.map((opt) => (
        <button
          ...
          className={[
            "rounded-lg px-2 py-1 text-[11px] font-semibold leading-tight transition",
  ...
  return (
    ...
    >
      ...
      <div className={isRail ? "border-t border-slate-200/80 pt-4" : "ml-auto"}>
        <div className={isRail ? "space-y-2" : "flex items-center gap-2"}>
          {!canApply ? (
            <p className={isRail ? "text-xs font-semibold text-amber-700" : "text-[11px] font-semibold text-amber-700"}>
              Select at least one {missingSelections.join(" and one ")} to apply.
            </p>
          ) : null}
          <button
            type="submit"
            disabled={!canApply}
            className={[
              "inline-flex items-center gap-1.5 bg-[linear-gradient(135deg,#0f172a,#14532d)] font-bold text-white transition",
              "disabled:cursor-not-allowed disabled:opacity-40",
              isRail
                ? "w-full justify-center rounded-2xl px-4 py-3 text-sm shadow-[0_18px_36px_rgba(15,23,42,0.18)] hover:-translate-y-0.5"
